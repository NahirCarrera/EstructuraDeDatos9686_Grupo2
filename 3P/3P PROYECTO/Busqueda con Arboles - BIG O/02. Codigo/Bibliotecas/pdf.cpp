...
static string handlePolygon(const vector<XY> &points, const string &theOperator){
   if(points.size() <= 2)
      return("");
...
static string handleRectangle(int x, int y, int width, int height, const string &theOperator){
   ostringstream out;
...
static string handleColor(unsigned char red,unsigned char green,unsigned char blue,const string &theOperator){
   ostringstream out;
   out <<
      (red   / 255.0) << " " <<
      (green / 255.0) << " " <<
...
static vector<XY> strokeEllipse(int xCenter, int yCenter, int xRadius, int yRadius)
{
   // ----------------
   //
   // Formula:
...
void PDF::showImage(const ImageInfo &info, int x, int y, double xScale, double yScale)
{
...
int PDF::stringWidth(Font currentFont, int currentSize, const string &text){
   int width    = 0;
...
vector<string> PDF::wrapText(const string &text, int maxWidth, bool rightJustify){
...
void PDF::drawEllipse(int xCenter, int yCenter, int xRadius, int yRadius){
   mCurrentPageContents += handlePolygon(
      strokeEllipse(xCenter, yCenter, xRadius, yRadius), OP_STROKE
   );
}

void PDF::fillEllipse(int xCenter, int yCenter, int xRadius, int yRadius){
   mCurrentPageContents += handlePolygon(
...
void PDF::setLineColor(unsigned char red, unsigned char green, unsigned char blue){
   mCurrentPageContents += handleColor(red, green, blue, OP_LINE_COLOR);
}

void PDF::setFillColor(unsigned char red, unsigned char green, unsigned char blue){
