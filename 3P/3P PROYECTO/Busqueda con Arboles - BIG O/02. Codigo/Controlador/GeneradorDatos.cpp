...
bool GeneradorDatos::buscarEnVector(const std::vector<std::string>& vec, const std::string& elemento) {
...
std::string GeneradorDatos::generarCedulaUnica(std::vector<std::string>& cedulasGeneradas) {
    std::string cedula;
...
std::string GeneradorDatos::generarNombreAleatorio() {
...
std::string GeneradorDatos::generarApellidoAleatorio() {
...
std::string GeneradorDatos::generarFechaNacimientoAleatoria() {
...
std::string GeneradorDatos::generarFechaHoraAleatoria() {
    int dia = rand() % 28 + 1;
    int mes = rand() % 12 + 1;
...
std::string GeneradorDatos::generarRegistrosAleatorios(const std::vector<std::string>& cedulasGeneradas) {
    std::string registro;
...
void GeneradorDatos::ejecutar(int cantidad) {
    ...
    if (registroArchivo.is_open()) {
        ...
        for (int i = 0; i < cantidad; i++) { // Cambia 1000 al número deseado de registros
